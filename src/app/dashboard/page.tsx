...
export default async function DashboardPage() {
  ...
  const [entriesCount, latestEntries] = await Promise.all([
    Promise.resolve(db.journalEntry.countByUserId(user.id)),
    Promise.resolve(db.journalEntry.findManyByUserId(user.id, 3)),
  ]);
