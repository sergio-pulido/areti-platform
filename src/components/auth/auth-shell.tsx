...
export function AuthShell({ title, subtitle, children }: AuthShellProps) {
  return (
    <div className="grid min-h-screen grid-cols-1 bg-night-950 text-sand-100 lg:grid-cols-[1fr_460px]">
      <section className="relative hidden overflow-hidden border-r border-night-800/80 lg:block">
        ...
        <div className="relative z-10 flex h-full flex-col justify-between p-12">
          <div>
            <p className="text-xs uppercase tracking-[0.35em] text-sage-200/90">
              Stoa + Garden
            </p>
          ...
          <div className="grid gap-4">
            {[
              ...
              <div key={item} className="rounded-2xl border border-night-700/60 bg-night-900/40 px-5 py-4 text-sm text-sand-200/90">
                {item}
              </div>
            ))}
          </div>
